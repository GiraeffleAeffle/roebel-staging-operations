...
def flux_bootstrap_objects(v,root,original):
    ...
    if v.TOWN_WORKSPACE.stage(v, root) in ('review', 'brief', 'multi-case', 'demo-login', 'discussion-context'):
        role=next(o for o in expected if o['kind']=='Role')
...
def verify(v,root):
    ...
    for name in ('resources.json','kustomization.yaml'):
        ...
        if name=='resources.json' and selected in ('review','brief','multi-case','demo-login','discussion-context'):
