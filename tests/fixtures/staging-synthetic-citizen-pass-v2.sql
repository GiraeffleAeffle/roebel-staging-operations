-- Forward-only identity rotation. Historical rows and migration 76 are unchanged.
-- Replaces only the three reviewed function definitions and schema metadata.
begin;
...
declare
  ...
  current_id text;
begin
  select migration_id into strict current_id
    from staging_participant_private.staging_participant_synthetic_adoption_schema_contract
   where singleton for update;
  if current_id not in ('20260902_staging_synthetic_citizen_adoption', '20260905_staging_synthetic_citizen_pass_v2') then
    raise exception 'STAGING_TEST_IDENTITY_ROTATION_SCHEMA_DRIFT';
  end if;
  for expected in select * from (values
    ('staging_participant_gateway_issue_synthetic_challenge', '7cff4c4ece26dca157630bae37e2ff44a91a3cc7abbde35cdbfdd03715038b2d', 'd95747275d761db7054145ce01743437127c7971a9e6bc1aee3de116f3706fb9'),
    ('staging_participant_gateway_accept_synthetic_adoption', '4c9bf49b759a72fcf472af667bd7126ababd8ff36581e6419fbcb670615b565b', 'c87ca05af9e73561d3976a0ce0976ab404b27d07e6514d574276dd38065bdb0e'),
    ('staging_participant_gateway_synthetic_adoption_preflight', 'e2a4372961f716a44531bd2ac710c4805f81240c3fac2201d171f5d205694984', '965b3f04f2438a5f72129083f92667d4325a3ee81f175a09e8b3843d102dee48')
  ) as pins(name, old_hash, new_hash) loop
...
begin
  ...
  if p_challenge is null or jsonb_typeof(p_challenge) is distinct from 'object'
     ...
     or v_contract is distinct from '0x4765cb681e8eb080b3191dd550e81eaa41907323'
...
update staging_participant_private.staging_participant_synthetic_adoption_schema_contract
   set migration_id = '20260905_staging_synthetic_citizen_pass_v2',
       database_schema_sha256 = 'sha256:c072fbc87a8fe6d4be9ef83359e919b639a5afddcef2a0dda337defad272462a',
       canonical_contract = $contract${"authorityBinding":"none","environment":"staging","identityContractSet":"gnosis-staging-test-v2","migrationId":"20260905_staging_synthetic_citizen_pass_v2","privateTables":["staging_participant_private.staging_participant_synthetic_citizen_challenges","staging_participant_private.staging_participant_synthetic_citizen_adoptions"],"publicProjection":"public_synthetic_citizen_adoption_projection_v1","realSchemasForbidden":["municipal_civic_eligibility_receipt_v1","citizen_topic_suggestion_adoption_request_v1","public_citizen_adoption_projection_v1","public_case_binding_receipt_v2"],"rpc":["staging_participant_gateway_issue_synthetic_challenge","staging_participant_gateway_consume_synthetic_challenge","staging_participant_gateway_resolve_synthetic_adoption_replay","staging_participant_gateway_accept_synthetic_adoption","staging_participant_gateway_read_public_synthetic_adoption"],"testCitizenNftContract":"0x4765cb681e8eb080b3191dd550e81eaa41907323","testOnly":true}
$contract$
 where singleton;

-- CREATE OR REPLACE preserves ownership and ACLs; restore the explicit grants too.
